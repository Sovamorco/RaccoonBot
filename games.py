...
from discord.ext.commands import Bot, Cog, hybrid_command
...
class Games(Cog):
    ...
    @hybrid_command(
        name="osuplayer",
        aliases=["op"],
        help="Команда для получения информации о игроке osu!standart",
        usage="osuplayer <ник/id>",
    )
    async def op_(self, ctx, *, nickname):
        api_link = "https://osu.ppy.sh/api/"
        params = {"k": self.bot.config["osu_key"], "u": nickname}
        async with ClientSession() as client:
        ...
        embed = Embed(
            ...
            url="https://osu.ppy.sh/users/{}".format(result["user_id"]),
        )
        embed.set_thumbnail(url="https://a.ppy.sh/{}".format(result["user_id"]))
        embed.add_field(
            name="Rank", value="{:,}".format(int(result["pp_rank"])), inline=False
        )
        embed.add_field(
            name="Country rank :flag_{}:".format(result["country"].lower()),
            value="{:,}".format(int(result["pp_country_rank"])),
            inline=False,
        )
        embed.add_field(
            name="PP", value="{:,}".format(round(float(result["pp_raw"]))), inline=False
        )
        embed.add_field(
            name="Accuracy",
        ...
        embed.add_field(
            name="Level", value=str(int(float(result["level"]))), inline=False
        )
        embed.add_field(
            name="Play Count",
    ...
    async def ops_(self, ctx, *, nickname):
        ...
        params = {"k": self.bot.config["osu_key"], "u": nickname}
        async with ClientSession() as client:
            plays = await client.get(
                api_link + "get_user_best", params=params, timeout=2
            )
        ...
        for i in range(len(plays)):
            params = {"k": self.bot.config["osu_key"], "b": plays[i]["beatmap_id"]}
            async with ClientSession() as client:
            ...
            accuracy = round(
                (
                    int(plays[i]["count300"]) * 300
                    + int(plays[i]["count100"]) * 100
                    + int(plays[i]["count50"]) * 50
                )
                / (
                    (
                        int(plays[i]["count300"])
                        + int(plays[i]["count100"])
                        + int(plays[i]["count50"])
                    )
                    * 3
                ),
            ...
            combo = "{:,} ({})".format(
                int(plays[i]["maxcombo"]),
                (
                    "FC"
                    if plays[i]["maxcombo"] == info["max_combo"]
                    else info["max_combo"]
                ),
            )
            ...
            name = "{}. {} [{}] ({})".format(
                i + 1, info["title"], info["version"], mods
            )
    ...
    @hybrid_command(
        name="svcard",
        help="Команда для поиска карты из Shadowverse",
        usage="svcard <запрос>",
    )
    async def svcard_(self, ctx, *, search):
        ...
        for card in cards:
            if all(
                term.lower() in cards[card]["searchableText"].lower() for term in terms
            ):
        ...
        if not results:
            ...
            if len(results) == 1:
                ...
                def verify(m):
                    ...
                    if m.content.isdigit():
                        return (
                            0 <= int(m.content) <= len(results)
                            and m.channel == text_channel
                            and m.author == user
                        )
                    canc = (
            ...
            race = (
                result["faction"]
                if not result["race"]
                else "{}/{}".format(result["faction"], result["race"])
            )
            embed.set_thumbnail(
                url="https://sv.bagoum.com/cardF/en/c/{}".format(result["id"])
            )
            embed.set_image(
                url="https://sv.bagoum.com/getRawImage/0/0/{}".format(result["id"])
            )
            ...
            if result["baseData"]["description"]:
                embed.add_field(
                    name="Описание",
                    value=result["baseData"]["description"].replace("<br>", "\n"),
                    inline=False,
                )
            embed.add_field(
                name="Flair",
                value=result["baseData"]["flair"].replace("<br>", "\n"),
                inline=False,
            )
            if result["hasEvo"]:
                evoembed = Embed(
                ...
                evoembed.set_thumbnail(
                    url="https://sv.bagoum.com/cardF/en/e/{}".format(result["id"])
                )
                evoembed.set_image(
                    url="https://sv.bagoum.com/getRawImage/1/0/{}".format(result["id"])
                )
                if result["evoData"]["description"]:
                    evoembed.add_field(
                        name="Описание",
                        value=result["evoData"]["description"].replace("<br>", "\n"),
                        inline=False,
                    )
                ...
                evoembed.add_field(
                    name="Flair",
                    value=result["evoData"]["flair"].replace("<br>", "\n"),
                    inline=False,
                )
    ...
    @hybrid_command(
        name="svart",
        help="Команда для поиска арта карты из Shadowverse",
        usage="svart <запрос>",
    )
    async def svart_(self, ctx, *, search):
        ...
        if len(results) == 1:
            ...
            for i in range(len(results)):
                content += "{}. {}\n".format(i + 1, results[i]["name"])
            embed = Embed(
                color=Color.dark_purple(), title="Выберите карту", description=content
            )
            embed.set_footer(
                text="Автоматическая отмена через 30 секунд\nОтправьте 0 для отмены"
            )
    ...
    @hybrid_command(name="svupdate", help="Команда для обновления базы данных карт")
    async def update_(self, ctx):
        try:
            ...
            return await ctx.send(
                "При обновлении базы данных карт произошла ошибка. Подробнее:\n{}".format(
                    e
                )
            )
