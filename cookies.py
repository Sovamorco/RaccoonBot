...
class Cookies(Cog):
        ...
        while True:
            try:
                ...
                for user_id, user in online.items():
                    await self.add(
                        user_id,
                        randrange(31, 35) if user["voice"] else randrange(11, 15),
                    )
    ...
    async def add(self, userid, amt):
        await self.bot.sql_client.sql_req(
            ...
            userid,
            randrange(289, 296) + amt,
            amt,
        )

    async def get(self, userid):
        cookies = await self.bot.sql_client.sql_req(
            "SELECT cookies FROM cookies WHERE id=%s", userid, fetch_one=True
        )
        if cookies is None:
            return None
    ...
    @hybrid_command(
        name="cookies", aliases=["points"], help="Команда для отображения печенек"
    )
    ...
    @hybrid_command(
        name="leaderboard", aliases=["lb"], help="Команда для отображения топа печенек"
    )
    async def leaderboard_(self, ctx):
        ...
        async def get_lb_entry(pos, obj):
            ...
            return "{}. **{}**: {:,} {}".format(
                pos + 1, user.name, obj["cookies"], sform(obj["cookies"], "печенька")
            )
        ...
        local_entries = islice(
            (
                entry
                for entry in cookies
                if ctx.guild.get_member(entry["id"]) is not None
            ),
            10,
        )
        local_lb = "\n".join(
    ...
    async def bj_(self, ctx, amt: int):
        ...
        Дилер берет {}
        Дилер выдает вам {}
        Дилер берет в закрытую""".format(
            fst[0], snd[0], trd[0]
        )
        embed = Embed(
            color=Color.dark_purple(), title="Ход игры", description=embedValue
        )
